...
parser.add_argument('--cluster', type=bool, default=False, help='whether want to use clustered endogeneous features')
...
dataset = FNNDataset(root='data', feature=args.feature, empty=False, name=args.dataset, transform=ToUndirected(), cluster=args.cluster)
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i, data in enumerate(train_loader):
            # This is used to make all the variables as zero, default setting accumulates all gradients.
            optimizer.zero_grad()
            if not args.multi_gpu:
                data = data.to(args.device)
            out = model(data)
            ...
            # Calculating negative log likelihood loss and performing backward propagation
            loss = F.nll_loss(out, y)
            loss.backward()

            # This performs single optimization step
            optimizer.step()

            # This item() method extracts the loss’s value as a Python float.
