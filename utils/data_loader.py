...
def cluster_feature(folder, feature='bert'):
    ...
    for key in mapping_embedding2cluster.keys():
        ...
        umap_embedding = umap.UMAP(n_neighbors=5, n_components=1, min_dist=0.0, metric='cosine').fit_transform(mapping_t)
...
def split(data, batch):
	...
	"""
	# we are finding the cumulative sum of number of occurrence of each node
	node_slice = torch.cumsum(torch.from_numpy(np.bincount(batch)), 0)

	# concatenating 0 at the front of the tensor node_slice which contains the cumulative sum of all the occurrences of
	# nodes. Node indices must start at 0 for every graph
...
def read_graph_data(folder, feature, cluster):
	"""
	PyG util code to create PyG data instance from raw graph data
	"""
	if cluster:
		cluster_feature(folder, feature)
		feature = feature + '_clustered'
	print(feature)
...
class FNNDataset(InMemoryDataset):
	...
	def __init__(self, root, name, feature='spacy', empty=False, transform=None, pre_transform=None, pre_filter=None, cluster=False):
		self.name = name
		self.root = root
	...
	def process(self):
		...
		self.data, self.slices = read_graph_data(self.raw_dir, self.feature, self.cluster)
